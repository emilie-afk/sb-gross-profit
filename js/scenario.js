...
export function allocateOrderShipping(scenarioLines) {
  ...
  for (const indices of byOrder.values()) {
    ...
    const weights = indices.map(i => scenarioLines[i].scenarioRevenue || 0);
...
export function summarizeScenario(lines, assumptions = {}) {
  ...
  const totalScenarioRevenue = r2(scen.reduce((s, l) => s + l.scenarioRevenue, 0));
  const totalAd = r2(totalScenarioRevenue * adRate);
  const adAlloc = allocateByWeight(totalAd, scen.map(l => l.scenarioRevenue));

  const laborInfo = allocateLabor({
    monthlyLabor: a.monthlyLabor, dateFrom: a.dateFrom, dateTo: a.dateTo });
  const laborAlloc = allocateByWeight(laborInfo.allocated, scen.map(l => l.scenarioRevenue));
  ...
  const currentProductRevenue = r2(enriched.reduce((s, l) => s + l.currentRevenue, 0));
  ...
  const coverage = costCoverage(lines);
